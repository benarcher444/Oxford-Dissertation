...
class Swimmer:
    ...
    def construct_matrices(self, theta, coords):
        ...
        self.P[2, :self.N] = - change_x * self.L * sin(theta)*cos(theta) * (xi - eta) - change_y * self.L * (- xi * cos(theta)**2 - eta * sin(theta)**2) + eta * self.L**2/2 * sin(theta)
        self.P[2, self.N:2*self.N] = - change_x * self.L * (xi*sin(theta)**2 + eta*cos(theta)**2) - change_y * self.L * sin(theta)*cos(theta) * (- xi + eta) - eta * self.L**2/2 * cos(theta)
